...
def parse_arguments() -> tuple[str, str, int]:
    ...
    """
    parser = argparse.ArgumentParser(description="Преобразует изображение в стиль пиксел-арт.")
    parser.add_argument("input", type=str, help="Путь к входному изображению")
    parser.add_argument("output", type=str, help="Путь для сохранения результата")
    parser.add_argument("--scale", type=float, default=0.1, help="Коэффициент масштабирования")
    args = parser.parse_args()
    return args.input, args.output, args.scale
...
def apply_pixel_art(img: np.ndarray, scale_factor: float = 0.1) -> np.ndarray:
    ...
    if small_h < 1 or small_w < 1:
        raise ValueError("Коэффициент масштабирования слишком мал для данного изображения.")
...
def main() -> None:
    input_path, output_path, scale_factor = parse_arguments()
    ...
    try:
        ...
        if img is None:
            raise ValueError(f"Не удалось загрузить изображение: {input_path}. Проверьте формат.")
