...
class AudioPlayerWindow(QMainWindow):
    ...
    def select_annotation_file(self):
        ...
        if file_path:
            try:
                ...
                if self.audio_paths:
                    for btn in [self.prev_btn, self.play_btn, self.next_btn, self.noise_btn]:
                        btn.setEnabled(True)
    ...
    def add_white_noise_to_current(self):
    ...
    def position_changed(self, position):
        if self.media_player.duration() > 0 and not self.progress_slider.isSliderDown():
