...
def download_piano_audio(download_folder, annotation_file, max_files=50):
    ...
    try:
        # Основные страницы с музыкой пианино
        urls = [
        ...
        for url in urls:
            ...
            try:
                ...
                # Метод 1: Ищем в JavaScript данных
                script_tags = soup.find_all('script')
                for script in script_tags:
                    if script.string:
                        script_text = script.string
                        
                        # Ищем JSON данные с ссылками на аудио
                        if 'previewUrl' in script_text or 'audioUrl' in script_text:
                            # Ищем URL в разных форматах
                            urls_found = re.findall(r'["\'](https?://[^"\']+?\.mp3)["\']', script_text)
                            all_audio_urls.extend(urls_found)
                            
                            # Ищем в JSON структурах
                            json_matches = re.findall(r'\{[^}]+"previewUrl"[^}]+\}', script_text)
                ...
                # Метод 2: Ищем прямые ссылки на аудио файлы
        ...
        # Убираем дубликаты и невалидные URL
        all_audio_urls = list(set(all_audio_urls))
        valid_audio_urls = [url for url in all_audio_urls if url.startswith('http') and '.mp3' in url]
        ...
        for i, audio_url in enumerate(valid_audio_urls[:max_files]):
            try:
                ...
                # Проверяем что это действительно MP3 файл
                if audio_response.headers.get('content-type') == 'audio/mpeg' or len(audio_response.content) > 1000:
        ...
        # Если всё еще не нашли, создаем демо-файлы но с реальными именами mixkit
...
def main():
    parser = argparse.ArgumentParser(description='Скачивание аудиофайлов с пианино с mixkit.co')
    parser.add_argument('--download-folder', type=str, required=True, 
