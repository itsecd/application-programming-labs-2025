    ...
    def download_sound_from_page(self, page_url, download_folder, min_duration=10):
        ...
        try:
            ...
            if audio_response.status_code == 200:
                ...
                print(f"Ошибка HTTP {audio_response.status_code} при скачивании: {audio_url}")
        except Exception as e:
            print(f"Ошибка скачивания {filename}: {e}")
        
        return None, None

    def download_sounds(self, download_folder, min_files=50, max_files=1000, min_duration=10):
        ...
        with open(annotation_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(['absolute_path', 'relative_path', 'filename', 'duration_seconds'])
            ...
            for i, page_url in enumerate(sound_pages):
                ...
                filepath, filename = self.download_sound_from_page(page_url, download_folder, min_duration)
                ...
                if filepath and os.path.exists(filepath):
                    ...
                    writer.writerow([absolute_path, relative_path, filename, duration_sec])
                    downloaded += 1
                    print(f"✅ Сохранён: {filename} ({duration_sec} сек) [{downloaded}/{max_files}]")
                else:
        ...
        if downloaded < min_files:
            print(f"⚠️  Внимание: скачано меньше минимального количества ({min_files})")
...
class SoundIterator:
    def __init__(self, annotation_source):
        ...
        if os.path.isfile(annotation_source):
            ...
            raise ValueError(f"Укажите существующий CSV-файл или папку: {annotation_source}")
...
def main():
    parser = argparse.ArgumentParser(description='Скачивание звуков с mixkit.co (только >10 секунд)')
    parser.add_argument('--download-folder', '-d', required=True, help='Папка для сохранения аудиофайлов')
    parser.add_argument('--min-files', type=int, default=50, help='Минимальное количество файлов')
    parser.add_argument('--max-files', type=int, default=100, help='Максимальное количество файлов')
