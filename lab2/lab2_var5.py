...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="ЛР: скачивание изображений по цвету и создание CSV-аннотации"
    )
    parser.add_argument(
        "--color",
        required=True,
        choices=["red", "yellow", "green", "blue"],
        help="Цвет изображений (red, yellow, green, blue)",
    )
    parser.add_argument(
        "--count",
        type=int,
    ...
    parser.add_argument(
        "--keyword",
        default="bird",
        help='Ключевое слово поиска (по умолчанию "bird")',
    )
...
def download_images(keyword: str, color: str, save_dir: Path, count: int) -> None:
    ...
    color_keywords = {
        "red": "cardinal bird",
        "yellow": "yellow warbler",
        "green": "parrot green",
        "blue": "blue jay",
    }
    keyword = color_keywords.get(color, keyword)
...
class PathIterator:
    ...
    def __init__(self, csv_path: Path | None = None, folder_path: Path | None = None) -> None:
        ...
        Внутри сразу заполняет список self.paths и обнуляет индекс.
        """
        if csv_path is None and folder_path is None:
            raise ValueError("Нужно указать путь к CSV или путь к папке.")

        self.paths: list[Path] = []

        # Если передан CSV — читаем из него
        if csv_path is not None:
            with csv_path.open("r", encoding="utf-8", newline="") as f:
                ...
                for row in reader:
                    ...
                    if abs_str:
                        self.paths.append(Path(abs_str))
    ...
    def __iter__(self):
        """
        iter() для итератора просто возвращает self.
        """
        return self
    ...
    def __next__(self):
        ...
        Если дошли до конца списка — бросает StopIteration.
        """
...
def main() -> None:
    ...
    if not (50 <= args.count <= 1000):
        raise StopIteration("Ошибка: --count должен быть в диапазоне от 50 до 1000 изображений.")
    ...
    for el in PathIterator(csv_path=Path(args.csv)):
        print(el)
