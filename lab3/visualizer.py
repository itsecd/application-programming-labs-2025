...
def plot_audio_comparison(
    ...
    if len(original.shape) == 1:  # Моно
        ...
        axes[0].plot(
            time_original, original[:, 0], color="blue", alpha=0.7, label="Левый канал"
        )
        axes[0].plot(
            time_original, original[:, 1], color="red", alpha=0.7, label="Правый канал"
        )
        axes[0].legend()
        axes[0].set_title(
            f"Оригинальное аудио (стерео): {os.path.basename(original_file)}"
        )
