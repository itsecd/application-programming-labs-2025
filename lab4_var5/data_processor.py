...
class DataProcessor:
    ...
    def load_annotation_data(self, annotation_file):
        ...
        self.df = self.df[['absolute_path', 'relative_path']]
    ...
    def add_brightness_column(self, brightness_values):
        ...
        self.df['brightness'] = brightness_values
        return self.df
    ...
    def sort_by_brightness(self, ascending=True):
        ...
        """
        if self.df is None or 'brightness' not in self.df.columns:
            raise ValueError("DataFrame не содержит колонку с яркостью")

        sorted_df = self.df.sort_values('brightness', ascending=ascending)
    ...
    def get_statistics(self):
        ...
        return {
            ...
            'min_brightness': round(self.df['brightness'].min(), 2),
            'max_brightness': round(self.df['brightness'].max(), 2),
            'mean_brightness': round(self.df['brightness'].mean(), 2),
            'median_brightness': round(self.df['brightness'].median(), 2)
        }
